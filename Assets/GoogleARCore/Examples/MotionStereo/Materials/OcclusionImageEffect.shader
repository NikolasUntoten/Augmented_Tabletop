    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                {
                    depthMeters =
                        tex2Dlod(_CurrentDepthTexture, float4(i.uv, 0, 0)).r *
                        ARCORE_MAX_DEPTH_MM;
                ...
                float virtualDepthMeters = LinearEyeDepth(
                ...
                float halfTransitionRangeMeters =
                    virtualDepthMeters * _TransitionSize * 0.5;

                // 0.0 - fully visible, 1.0 - fully occluded.
                float occlusionAlpha = smoothstep(
                    depthMeters - halfTransitionRangeMeters,
                    depthMeters + halfTransitionRangeMeters,
                    virtualDepthMeters);

                // Masks out only the fragments with virtual objects.
                occlusionAlpha *= saturate(maxVirtualDepth -
                                           virtualDepthMeters);
